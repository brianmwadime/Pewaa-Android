...
public class ContactsService {
    ...
    private APIContact initializeApiContact() {
        if (mApiContact == null) {
            mApiContact = this.mApiService.RootService(APIContact.class, PreferenceManager.getToken(WhatsCloneApplication.getAppContext()), EndPoints.BASE_URL);
        }
        return mApiContact;
    }
    ...
    public Observable<RealmResults<ContactsModel>> getAllContacts() {
        return realm.where(ContactsModel.class).notEqualTo("id", PreferenceManager.getID(WhatsCloneApplication.getAppContext())).equalTo("Exist", true).findAllSorted("Linked", Sort.DESCENDING, "username", Sort.ASCENDING).asObservable();
    }
    ...
    public Observable<RealmResults<ContactsModel>> getLinkedContacts() {
        return realm.where(ContactsModel.class).notEqualTo("id", PreferenceManager.getID(WhatsCloneApplication.getAppContext())).equalTo("Exist", true).equalTo("Linked", true).findAllSorted("username", Sort.ASCENDING).asObservable();
    }
    ...
    public Observable<RealmResults<StatusModel>> getAllStatus() {
        return realm.where(StatusModel.class).equalTo("userID", PreferenceManager.getID(WhatsCloneApplication.getAppContext())).findAllSorted("id", Sort.DESCENDING).asObservable();
    }
    ...
    public Observable<StatusModel> getCurrentStatusFromLocal() {
        return realm.where(StatusModel.class).equalTo("userID", PreferenceManager.getID(WhatsCloneApplication.getAppContext())).equalTo("current", 1).findFirst().asObservable();
    }
    ...
    private ContactsModel copyOrUpdateContactInfo(ContactsModel contactsModel) {
        ...
        if (UtilsPhone.checkIfContactExist(WhatsCloneApplication.getAppContext(), contactsModel.getPhone())) {
            realm.beginTransaction();
