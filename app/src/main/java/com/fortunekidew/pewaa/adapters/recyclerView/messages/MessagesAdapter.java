...
import com.fortunekidew.pewaa.api.FilesDownloadService;
import com.fortunekidew.pewaa.api.FilesUploadService;
import com.fortunekidew.pewaa.app.AppConstants;
import com.fortunekidew.pewaa.app.EndPoints;
import com.fortunekidew.pewaa.helpers.AppHelper;
import com.fortunekidew.pewaa.helpers.Files.DownloadFilesHelper;
...
import com.fortunekidew.pewaa.helpers.images.BlurTransformation;
...
public class MessagesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void UpdateMessageItem(MessagesModel messagesModel) {
    ...
    public void updateMessageStatus(int status, int messageId) {

        for (int i = 0; i < mMessagesModel.size(); i++) {
            MessagesModel model = mMessagesModel.get(i);
            if (messageId == model.getId()) {
                Realm realm = Realm.getDefaultInstance();
                mMessagesModel.remove(i);
                MessagesModel messagesModel = realm.where(MessagesModel.class).equalTo("id", messageId).findFirst();
                mMessagesModel.add(i, messagesModel);
                notifyDataSetChanged();
                /*
                MessagesViewHolder messagesViewHolder = (MessagesViewHolder) viewHolder;
                messagesViewHolder.showSent(status);*/
                realm.close();
                break;
            }
        }
        ...
        void setImageFile(MessagesModel messagesModel) {
            Transformation blurTransformation = new Transformation() {
                ...
                public Bitmap transform(Bitmap source) {
                    Bitmap blurred = BlurTransformation.fastblur(mActivity, source, 10);
                    source.recycle();
                    return blurred;
                }
