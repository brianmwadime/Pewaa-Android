...
import com.fortunekidew.pewaa.presenters.MessagesPresenter;
...
public class MessagesActivity extends AppCompatActivity implements EmojiconGridFragment.OnEmojiconClickedListener, EmojiconsFragment.OnEmojiconBackspaceClickedListener, LoadingData, RecyclerView.OnItemTouchListener, ActionMode.Callback, View.OnClickListener {
    ...
    @Bind(R.id.keyboradBtn)
    ImageButton keyboradBtn;
    @Bind(R.id.toolbar)
    Toolbar toolbar;
    ...
    private void setRecipientState() {
        try{
            if (!isGroup) {
                ...
                if (contactsModel.getUserState() == null)
                    ...
                    lastVu.setText(contactsModel.getUserState());
            }
        }catch (Exception e){
            AppHelper.LogCat("Exception MessageActivity "+e.getMessage());
            lastVu.setText(getString(R.string.isOffline));
        }
    ...
    public void initializerView() {
        ...
        messageWrapper.setOnClickListener(v1 -> {
            if (emoticonShown) {
                emoticonShown = false;
                EmoticonButton.setVisibility(View.VISIBLE);
                keyboradBtn.setVisibility(View.GONE);
                final Animation animation = AnimationUtils.loadAnimation(this, R.anim.slide_down);
        ...
        EmoticonButton.setOnClickListener(v -> {
            if (!emoticonShown) {
                ...
                keyboradBtn.setVisibility(View.VISIBLE);
        ...
        keyboradBtn.setOnClickListener(v -> {
            messageWrapper.performClick();
        });
...
//                    NotificationsManager.cancelNotification(recipientId);
        ...
        if (resultCode == RESULT_OK) {
            ...
            switch (requestCode) {
                ...
                case AppConstants.UPLOAD_VIDEO_REQUEST_CODE:
                    FileVideoPath = FilesManager.getPath(PewaaApplication.getAppContext(), data.getData());
                    ...
                    FileVideoThumbnailPath = FilesManager.getPath(PewaaApplication.getAppContext(), Uri.fromFile(fileVideo));
                    sendMessage();
                    break;
                ...
                case AppConstants.UPLOAD_AUDIO_REQUEST_CODE:
                    FileAudioPath = FilesManager.getPath(PewaaApplication.getAppContext(), data.getData());
                ...
                case AppConstants.UPLOAD_DOCUMENT_REQUEST_CODE:
                    FileDocumentPath = FilesManager.getPath(PewaaApplication.getAppContext(), data.getData());
     ...
     */
    private void unSentMessagesForARecipient(String recipientID, boolean forFiles) {
        Realm realm = Realm.getDefaultInstance();
        List<MessagesModel> messagesModelsList = realm.where(MessagesModel.class)
                .equalTo("status", AppConstants.IS_WAITING)
...
//                    NotificationsManager.cancelNotification(recipientId);
     ...
     */
    private boolean checkIfUserIsExist(String id, Realm realm) {
    ...
    public void onEventMainThread(Pusher pusher) {
        ...
        switch (pusher.getAction()) {
            ...
            case "messages_delivered":
                mMessagesAdapter.updateMessageStatus(AppConstants.IS_DELIVERED, messageId);
                break;
            case "new_message_sent":
                mMessagesAdapter.updateMessageStatus(AppConstants.IS_SENT, messageId);
                break;
            case "messages_seen":
                mMessagesAdapter.updateMessageStatus(AppConstants.IS_SEEN, messageId);
                break;
            ...
            case "NewUserNotification":
                ...
                if (newUserNotification.getRecipientId() == recipientId) {
                    ...
                    if (newUserNotification.getFile() != null) {
                        NotificationsManager.showUserNotification(PewaaApplication.getAppContext(), messagingIntent, newUserNotification.getPhone(), newUserNotification.getFile(), recipientId, newUserNotification.getImage());
                    } else {
                        NotificationsManager.showUserNotification(PewaaApplication.getAppContext(), messagingIntent, newUserNotification.getPhone(), newUserNotification.getMessage(), recipientId, newUserNotification.getImage());
                    }
                }

                break;
            case "NewGroupNotification":
                ...
                if (newGroupNotification.getGroupID() == groupID) {
                    ...
                    if (newGroupNotification.getFile() != null) {
                        NotificationsManager.showGroupNotification(PewaaApplication.getAppContext(), messagingIntent, newGroupNotification.getGroupName(), newGroupNotification.getMemberName() + " : " + newGroupNotification.getFile(), newGroupNotification.getGroupID(), newGroupNotification.getImage());
                    } else {
                        NotificationsManager.showGroupNotification(PewaaApplication.getAppContext(), messagingIntent, newGroupNotification.getGroupName(), newGroupNotification.getMemberName() + " : " + newGroupNotification.getMessage(), newGroupNotification.getGroupID(), newGroupNotification.getImage());
                    }
                }
